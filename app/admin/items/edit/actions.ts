...
/**
 * Item image metadata actions (NPC-style)
 * Storage upload is handled on the client with the supabase *client*.
 * These just update fields on public.items.
 */
export async function itemSetImageMetaAction(itemId: string, imageAlt: string | null) {
...
export async function itemClearImageMetaAction(itemId: string) {
  if (!isUuid(itemId)) redirect("/admin/items/edit?err=bad_id");
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      image_url: null, // optional; keep if you want the legacy field cleared too
    })
    .eq("id", itemId);
...
export async function deleteItemAction(formData: FormData) {
  ...
  if (error) {
    ...
    redirect(`/admin/items?err=${encodeURIComponent(error.message ?? "delete_failed")}`);
  }
