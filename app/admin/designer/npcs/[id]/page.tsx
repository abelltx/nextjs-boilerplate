...
function extractIdFromPath(pathname: string): string | null {
  // matches /admin/designer/npcs/<uuid> (with or without trailing slash)
  const m = pathname.match(/\/admin\/designer\/npcs\/([^\/\?\#]+)/i);
  if (!m?.[1]) return null;
  const candidate = decodeURIComponent(m[1]).trim();
  return isUuidLoose(candidate) ? candidate : null;
}
...
export default async function EditNpcPage({ params }: { params: any }) {
  // 1) Try normal dynamic param
  let id = normalizeId(params?.id);

  // 2) Fallback: derive from headers (Vercel page requests often include x-forwarded-uri)
  if (!id || id === "undefined" || !isUuidLoose(id)) {
    const h = await headers();

    const xForwardedUri = h.get("x-forwarded-uri"); // often like "/admin/designer/npcs/<uuid>"
    const referer = h.get("referer"); // sometimes full URL

    let pathFromHeader: string | null = null;

    if (xForwardedUri) {
      pathFromHeader = xForwardedUri;
    } else if (referer) {
      try {
        pathFromHeader = new URL(referer).pathname;
      } catch {
        // ignore
      }
    }
    ...
    if (pathFromHeader) {
      id = extractIdFromPath(pathFromHeader);
    }

    // Still no id → show diagnostics
    if (!id) {
      return (
        <ErrorPanel
          title="Invalid NPC id"
          message="This NPC link is invalid (route params missing)."
          details={{
            params,
            xForwardedUri,
            referer,
  ...
  if (!npc) {
    return (
      <ErrorPanel
        title="NPC not found"
        message="This NPC does not exist (or is not accessible)."
        details={{ id }}
      />
    );
  }
